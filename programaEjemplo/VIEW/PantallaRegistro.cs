        ...
        private List<ventaRT.clases.detalle_registro> ObtenerDetalle(string docnum)
        {
            ...
            if (rsCards.RecordCount != 0)
            {
                ...
                for (int i = 1; !rsCards.EoF; i++)
                {
                    ...
                    detalles.codArti = fields.Item(Constantes.View.DET_RVTabla.U_codArti).Value.ToString();
                            ...
                            if (Lineas.Count != 0)
                            {
                                ...
                                foreach (var item in Lineas)
                                {
                                    if (oUserTableDE.GetByKey(item.code))
                                    {
                                        ...
                                        oUserTableDE.UserFields.Fields.Item(Constantes.View.DET_RVTabla.U_codArti).Value = item.codArti;
                                        oUserTableDE.UserFields.Fields.Item(Constantes.View.DET_RVTabla.U_codClie).Value = item.codClie;
